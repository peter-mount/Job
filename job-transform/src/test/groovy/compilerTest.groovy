...
def destDir = new File( project.basedir, "target/generated-test-sources/groovy/uk/trainwatch/job/transform" );
mkdir(destDir);
...
sourceDir.eachDirRecurse {
    it.eachFileMatch( ~/.*\.job/ ) {
        ...
        // Limit to /transform/*.job files
        if((m = n =~ /transform\/([a-zA-Z][a-zA-Z_]*?)([0-9]*?).job$/))
            tests << m.group(1);
    }
}
...
tests.each {
    test ->
        ...
        new File( "$destDir/"+className+".java" ).withWriter { out ->
            out.println "package uk.trainwatch.job.transform;";
            out.println "";
